...
function get_available_dbs_list( $all = true ) {
	global $dbdir;
	$files = scandir($dbdir);
...
function get_db_connection($db) {
	...
	$dbh;
        ...
        error_log("Get db connection to $dbpath in $dbdir");
	try {
...
function get_databases_data($all = false) {
	$dbs = get_available_dbs_list( $all );
	$data = array();
	foreach ($dbs as $db) {
		$dbh = get_db($db);
